...
<center>
    <h1>商品详情</h1>
	<form id="editform" action="<%=request.getContextPath()%>/items/editItemsSubmit.action?pageNo=1" method="post">
		<fieldset>
		<div>
					<label for="itemsId">商品ID：</label> <input type="text"
						name="itemsId" id="itemsId" value="${itemsCustom.id }" readonly="readonly"/>
				</div>
				<div>
				...
				<div>
					<label for="detail">商品详情：</label> <textarea  name="detail"
						id="detail" rows="3" cols="30" >${itemsCustom.detail }</textarea>
				</div>
				<br/>
				<br/>
				<div>
					<label for="createtime">商品创建时间：</label> <input type="text"
						name="createtime" id="createtime" value="<fmt:formatDate value="${itemsCustom.createtime }"  pattern="yyyy-MM-dd"/>" />
				</div>
				<br/>
				<br/>
				<div>
